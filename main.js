function uuid() {
  ...
  function createTodoElement(todo) {
    ...
    todoItem.classList.add(
      'group','flex','justify-between','py-3','px-2.5', 'max-w-lg', 'mx-auto','rounded-lg','hover:bg-purple-300', 'hover:text-white'
    );
    ...
    todoActions.classList.add(
      'flex','gap-3','invisible','group-hover:visible'
    );
  ...
  function editTodoItem(id, todoTitle) {
    ...
    const updatedTitle = prompt('Edit your Todo list here:', todoToEdit.title);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const todos = todo_db.sort((a, b) => a.created_at - b.created_at);
